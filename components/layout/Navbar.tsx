    ...
    <nav className="absolute top-10 z-40 w-full flex items-center justify-between px-4 py-4 lg:px-10">
      ...
      {pathname !== "/" && (
        <div className="absolute left-1/2 transform -translate-x-1/2 top-0 flex justify-center items-center z-10">
          <Link href="/" className="w-[100px] h-[100px] lg:w-[180px] lg:h-[180px] block">
            <Image
              src={logo}
              alt="Company Logo"
      ...
      >
        <ul className="flex flex-col gap-8 list-none uppercase">
          {["Home", "Services", "Projects", "Contact"].map((label, index) => {
            ...
            return (
              <li key={index} onClick={closeMenu} className="relative text-lg tracking-widest group">
                <Link
                  href={href}
        ...
        <div className="flex flex-row gap-4 mt-8">
          <a href="mailto:hello@alleywaydxb.com" className="text-eggshell hover:text-tealCustom transition text-2xl">
            <FaEnvelope />
          </a>
